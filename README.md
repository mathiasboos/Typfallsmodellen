...
| 4. Advanced mode | done | the workbook's second mode: thirty-four of the `Adv_settings` in the manual's own groups (including partial withdrawal of the public pension, up to four children's Barnår birth years, and the ITP 1/SAF-LO flexpension premium), the Indata_lista wage path as an editable grid, and the PGB sheet's pension-qualifying amounts as a second grid — sickness/activity compensation typed in kronor, conscription and study computed from a date range and a semester count the same way the sheet itself computes them |
...
| Conscription and study's own PGB arithmetic | 15 unit tests pinning the day-split across one, two and three calendar years by hand, and the browser reading back a date range's own Dagar/PGB kronor columns and a semester count's own kronor column, both grouped under a named header in the grid itself | exact day counts and kronor at every split; a 1998 conscription period and a 2005 semester both raise the pension on their own, distinct from each other and from a typed sickness/activity amount |
