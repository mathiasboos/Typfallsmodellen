...
| 7. Mikrosim *(ported from the workbook's own Mikrosim sheet)* | done | a third top-level view: a batch table of independent typfall, each a full row of nine inputs, all twelve output columns computed live as each row is edited, added or imported, add/remove rows, CSV import/export, and a stacked column chart below the table breaking each row's pension into its seven components |
