...
| 7. Mikrosim *(ported from the workbook's own Mikrosim sheet)* | done | a third top-level view: a batch table of independent typfall, each a full row of nine inputs, with a "Beräkna" button that fills in all twelve output columns at once, add/remove rows, and CSV import/export |
...
| Mikrosim, driven in the same browser | adding and removing rows, editing one row's salary and scheme then clicking "Beräkna", CSV export and re-importing that same file, a file missing a required column, and a row with an invalid scheme next to a valid one | each row's own output columns come from its own inputs only, editing a row blanks its own results without touching another row's, a bad file is refused by name with the table left untouched, and an invalid row is flagged and skipped while the rest still calculate |
